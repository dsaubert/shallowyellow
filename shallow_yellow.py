...
class shallow_yellow:
    def __init__(self, input_image_size, available_actions):
        ...
        self.memories = deque(maxlen = 50000) #How long should the AI hold onto memories? Once the deque is full, it begins forgetting old memories from when the AI sucked
    ...
    def perform_memory_replay(self, sample_size):
        ...
        for current_state, chosen_action, reward, game_over, next_state in memory_sample:
            ...
            q_function_target[0][chosen_action] = chosen_action_target_reward #update the reward of the chosen action, with unchanged other actions
...
def play_episode(env, SY, game_over, current_state):
    for t in range(50000):
        ...
        next_state = SY.pre_process_new_frame(next_frame)
